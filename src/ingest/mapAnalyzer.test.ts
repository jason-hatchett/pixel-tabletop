...
describe("analyzeMap", () => {
  ...
  it("simplifies a diagonal boundary to a few segments (no staircase)", () => {
    const w = 80;
    const h = 80;
    const data = new Uint8ClampedArray(w * h * 4);
    for (let i = 0; i < w * h; i++) {
    ...
    // A right triangle: two axis legs + one diagonal hypotenuse.
    for (let y = 20; y < 70; y++)
      for (let x = 20; x < 70; x++)
        if (x - 20 + (y - 20) <= 40) {
          const p = (y * w + x) * 4;
    ...
    const { walls } = analyzeMap({ data, width: w, height: h }, { mmPerPx: 1 });
    // ~3 sides, not dozens of stair-steps down the hypotenuse.
